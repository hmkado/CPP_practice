...
class Robot{
    ...
    void initializeSearch(){
        ...
        g_cost.assign(sharedMap->getRow(),std::vector<int>(sharedMap->getCol(),INT_MAX));
    }
    int manhattan(const std::tuple<int, int>& location){
        int x_1, y_1, x_2,y_2;
        std::tie(x_1,y_1) = location;
        std::tie(x_2,y_2) = goal_point;
    ...
    void searchPath(){
        ...
        bfs_queue.push(Node(g_cost[std::get<0>(current_location)][std::get<1>(current_location)]+this->manhattan(current_location),current_location));
            ...
            for (auto& move : moves){
                ...
                && std::get<0>(moved_location) < sharedMap->getRow() && std::get<1>(moved_location) < sharedMap->getCol()){
                    ...
                    if (new_cost < g_cost[std::get<0>(moved_location)][std::get<1>(moved_location)]){
                        ...
                        bfs_queue.push(Node(new_cost + this->manhattan(moved_location),moved_location));
                    }
                }
            }
        }
